...
numSymbols = 100;   % Number of BPSK symbols
snr = 15;           % Signal-to-noise ratio in dB
% Modulation parameters
modulationIndex = 1;  % BPSK modulation index
% System parameters
...
% Simulate BPSK modulation and apply Kalman filter
for k = 1:numSymbols
    % BPSK modulation
    data = randi([0, 1]);
    modulatedSymbol = exp(1i*pi*(2*data-1)*modulationIndex);
    ...
    % Add noise
    noise = 10^(-snr/20) * (randn(1) + 1i * randn(1));

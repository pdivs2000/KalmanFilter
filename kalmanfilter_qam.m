...
numSymbols = 100;   % Number of QAM symbols
snr = 15;           % Signal-to-noise ratio in dB
% Modulation parameters
M = 16;             % QAM order (16-QAM in this case)
% System parameters
...
% Simulate QAM modulation and apply Kalman filter
for k = 1:numSymbols
    % QAM modulation
    data = randi([0, M-1]); % M possible symbols in QAM
    modulatedSymbol = qammod(data, M, 'UnitAveragePower', true);
    ...
    % Add noise
    noise = 10^(-snr/20) * (randn(size(receivedSymbol)) + 1i * randn(size(receivedSymbol)));
